...
class WhosTvParseTests(unittest.TestCase):
    ...
    def test_search_logs_in_uploads_polls_and_parses(self):
        ...
        self.assertEqual(result["matches"][0]["code"], "GANA-2823")
        self.assertEqual(client.session.post.call_args_list[0].args[0], "https://whos.tv/api/login")
        self.assertEqual(client.session.post.call_args_list[1].args[0], "https://whos.tv/upload-search")
    ...
    def test_search_waits_while_status_is_numeric_pending(self):
        ...
        client.session.post = mock.Mock(side_effect=[login, upload])
        client.session.get = mock.Mock(side_effect=[pending, ready, page])
        with tempfile.NamedTemporaryFile(suffix=".jpg") as image:
            result = client.search(image.name)
        self.assertEqual(result["matches"][0]["code"], "GANA-2823")
    ...
    def test_search_reauthenticates_once_when_upload_redirects_to_login(self):
        ...
        page = mock.Mock(status_code=200, text=RESULT_HTML); page.raise_for_status = mock.Mock()
        client.session.post = mock.Mock(side_effect=[ok, denied, ok, upload])
        client.session.get = mock.Mock(side_effect=[status, page])
        with tempfile.NamedTemporaryFile(suffix=".jpg") as image:
            result = client.search(image.name)
        self.assertEqual(result["matches"][0]["code"], "GANA-2823")
        self.assertEqual(sum(c.args[0].endswith('/api/login') for c in client.session.post.call_args_list), 2)
